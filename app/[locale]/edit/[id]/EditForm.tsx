...
export default function EditForm({ tForm, tEdit }: { tForm: any, tEdit: any }) {
    ...
    const fetchPost = async () => {
      try {
        ...
        if (err.message.includes('권한')) {
          setTimeout(() => router.push(`/${locale}/my-posts`), 2000);
        }
      } finally {
  ...
  if (isLoadingData) {
    return <div className="min-h-screen flex items-center justify-center">{tEdit.loading}</div>;
  }
  if (!post) {
    return <div className="min-h-screen flex items-center justify-center text-red-500">{error || tEdit.notFound}</div>;
  }
    ...
    <main className={`min-h-screen p-4 sm:p-8`}>
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="bg-white p-6 sm:p-8 rounded-xl border border-gray-200 space-y-6 h-fit">
          <h1 className="text-2xl font-black text-center text-gray-900">{tEdit.title}</h1>
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div><label htmlFor="thumbnail-upload" className="block text-sm font-medium text-gray-700 mb-1">{tForm.thumbnail}</label><input id="thumbnail-upload" type="file" accept="image/*" onChange={handleThumbnailUpload} disabled={isThumbnailUploading} className={fileInputStyle} />{isThumbnailUploading && <p className="text-xs text-gray-500 mt-2">{tForm.thumbnailUploading}</p>}</div>
            ...
            <div className="space-y-6 rounded-md border border-gray-200 p-4">
              ...
              {(visibility === 'password') && (
                <div>
                  ...
                  <input id="password" type="password" value={password} onChange={(e) => setPassword(e.target.value)} className={inputStyle} placeholder={tForm.passwordEditPlaceholder} />
                </div>
              )}
              <div>
            ...
            <button type="submit" disabled={isSubmitting || isUploading || isThumbnailUploading} className="w-full px-4 py-2.5 text-base font-semibold text-white bg-gray-800 rounded-md hover:bg-gray-900 disabled:bg-gray-400">{isSubmitting ? tForm.updating : tForm.updateButton}</button>
          </form>
